...
- **Create New / Filter** — a single entry point to create a Task, Project, Product, or Area, plus filtering by area/product/project. A "Via .csv" tab bulk-creates any of those four from a spreadsheet: download a template for the type you want, fill in a row per record (referencing an existing area/product/project by its exact title, not an internal id), and import the same file back. Runs every row through the same creation logic as everything else — partial success is reported per row, so one bad row doesn't block the rest.
