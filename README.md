...
- **Local Mode** — connect a local folder (File System Access, Chromium desktop only) that a small watcher script on your own machine polls (fast at first, backing off to every 5 seconds). Chat then runs entirely against a model you host yourself — no API key, no outbound HTTP call at all. Only the live conversation itself goes through the polled prompt/response files; the static system prompt and tool catalog are written once when the folder connects (`VAEA_SYSTEM_PROMPT.md`/`VAEA_TOOL_CATALOG.json`), and the live workspace snapshot is refreshed once per turn (`workspace-data.json`), so a Claude Code relay (or any watcher) reads both straight off disk instead of them being re-sent every round. **Settings → Resources → Local Mode setup guide** (`src/pages/LocalModeSetupGuidePage.jsx`) walks through the file-protocol spec and an example watcher script.

Both work in every deployment shape this README describes, including the standalone distributables below — Settings has no login gate, so configuring an API key or a Local Mode connection makes chat work fully even in an offline, no-account copy that the default Base44-hosted path can't reach.
